...
function checkAuthorization(){
    ...
    if (!$authToken && substr($_REQUEST['r'], 0, 15) == 'songs/songData/') $authToken = $_REQUEST['a'];
    $token = $_SESSION['token'];
...
function getAudioSource(int $songId, bool $full = false){
    ...
    if (!file_exists("..$rv")) createSampleFile($sampleInfo);
...
function getAuditLog(int $fromId){
    $ds = new DataService;
    $rv = $ds->getAuditLog($fromId);
    return $rv;
}
...
function getUserId(bool $throwException = true){
    $rv = $_SESSION['userId'];
       ...
       if (!$rv)
        if ($throwException) throw new Exception('Unauthorized', 401); else $rv = 0;
...
function rejectApplication(int $appId): int {
 ...
 function sendApprovalEmail(int $appId){
    ...
    $subject =  "Member Notification Email";
    $recipient =  $app->Email;
    $message = "<html><body><table width='100%'><tr><td width='100%'><font face='Verdana' size='2'>";
    $message .= "Dear $app->LastName, $app->FirstName" . ",<br><br>";
    $message .=  "You have just become an official member of $title. Congratulations and welcome to $title.<br><br>";
    $message .= "You are now ready to Login<br><br>";
...
$mail->IsSMTP();                                      // set mailer to use SMTP
$mail->Host = "secure177.inmotionhosting.com";  // specify main and backup server
$mail->SMTPAuth = true; 
...
$mail->Username = "support@recorddrop.com";
$mail->Password = "uObi48u})^j4";
$mail->Port = 465;
...
if($status_of_to[0])
{
	if(!$mail->Send())
	{
       insertAuditLog(getUserId(), 'Sent Email Error', json_encode($mail));
...
function updateApplicationStatus(int $appId, string $status): int {
 ...
 function updateSongs($song){
    ...
    $username = $_SESSION['username'];
    if($song->imageInput != ""){
    $sitepath="http://38.110.122.68/recorddrop1/images/artist_images";
...
function getsubmitdistribution($song){
 ...
 function sendSonguploadedEmail($user_id){
    ...
    $message = "<html><body><table width='100%'><tr><td width='100%'><font face='Verdana' size='2'>";
    $message .= "Dear " . $appdata->FirstName." ". $appdata->LastName. ",<br><br>";
    $message .=  "The song(s) that you uploaded has been received and is currently under review by our Quality Assurance department. recorddrop.com requires that all songs submitted must meet minimum standards of audio quality. During the review process all songs uploaded are checked for an abundance of things such as pops, white noise, muffled or incoherent words or music, or just plain bad audio quality. Although songs that do not pass QA are not distributed you will however retain the same number of songs that you had in your package prior to uploading. You will be receiving e-mail shortly letting you know that status of your song. In the mean time if you have questions you may contact us at  <a href='mailto:customercare@recorddrop.com'>customercare@recorddrop.com</a>.";
    $message .=  "<br><br>Sincerely,<br>Member Services</p></td></tr></table></body></html>";
    $headers =  "From: $title <$from>\n";
    $headers .=  "Content-Type: text/html; charset=iso-8859-1\n"; // Mime type
    //              mail($recipient, $subject, $message, $headers);
    smtpmail($from,$recipient,$subject,$message);
...
function getRejectsong($songId,$UserId){
 ...
 function sendSongrejectEmail($UserId){
    ...
    $subject =  "Your Song has been uploaded";
    $recipient =  $appdata->Email;
    //$recipient = "ketul.ketul.patel@gmail.com";
    $message = "<html><body><table width='100%'><tr><td width='100%'><font face='Verdana' size='2'>";
    $message .= "Dear " . $appdata->FirstName." ". $appdata->LastName. ",<br><br>";
    $message .=  "Dear recorddrop.Com Member,<p>Thank you for choosing recorddrop.Com to promote your music. Unfortunately, your song has not been submitted for review to recorddrop.Com DJs due to the following reason:<p>
        The audio quality of your file did not meet the minimum quality standards established by our QA Department.
        Your song may have contained pops, hiss, white noise, static, muffled audio, or the MP3 file you uploaded was not at least 128K, which is the minimum quality needed in order for DJs to play it at their clubs or radio stations.<p>
        Although your song has not been distributed, you still retain the right to upload the same number of songs that you had in your package prior to uploading.  At any time, you may submit this song, or any other song for review by our recorddrop.Com DJs.<p>
        For tips on how you may improve your audio quality, please visit the Help section of our web site under the section �Audio Tips�.<p>
        Rest assured, you are a valued recorddrop.Com member and it is our goal to continue providing you with the best online music promotion and distribution service that the web has to offer.<p>
        Sincerely,<p>Customer Service";
    $message .=  "</p></td></tr></table></body></html>";
