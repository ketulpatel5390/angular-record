...
class DataService {
    ...
    public function getAuditLog(int $skip, int $take){
        $dbh = $this->getDb();
    ...
    public function getLibrary(int $userId, int $skip, int $take,$sortbygener, $sortkey,$sortreverse){
        ...
        if($sortbygener != "" && $sortkey != "" ){
            ...
            $sql="SELECT  s.SongId, s.UserId, u.Username, s.SongFile, s.SongPath, s.SongTitle, s.ArtistName, s.Duration, Go.Name Genre,s.SubGenre, s.Label, s.State, s.Country, s.Website, s.CD, s.Vinyl, s.MIDownload, s.Status, 
                       s.tire2status, s.Region, s.Dist_Type, s.DJAssociation, s.Normal_Dist, s.DistDate, s.artist_image,
                        ...
                        s.Filename , sf.OverallRating,s.sociallink, s.Facebook_link, s.Twitter_link, s.Spotify_link, s.iTunes_link,
       ...
       /* $sql = "SELECT  s.SongId, s.UserId, u.Username, s.SongFile, s.SongPath, s.SongTitle, s.ArtistName, s.Duration, Go.Name Genre,s.SubGenre, s.Label, s.State, s.Country, s.Website, s.CD, s.Vinyl, s.MIDownload, s.Status, 
        ...
        foreach ($rv as $rvvalue) {
            ...
            if($rvvalue['sociallink'] == 'Y' ){

                $sql1 = "SELECT * FROM application where UserId = ".$rvvalue['UserId'];
                $sth1 = $dbh->prepare($sql1);
                $sth1->execute();
                $appobj = $sth1->fetchObject();
                //echo "<pre>";
                //print_r($appobj);
                //return $rv->total;
                //youtube link set to Spotify_link and instagram link set to iTunes_link
                
                $rvvalue['Facebook_link'] = $appobj->facebook;
                $rvvalue['Twitter_link'] = $appobj->twitter;
                $rvvalue['Spotify_link'] = $appobj->youtube;
                $rvvalue['iTunes_link'] = $appobj->instagram;
                
            }
            if($rvvalue['albumId'] != ""){
                $sql1 = "SELECT album_image,album_image_path FROM album 
                      WHERE album_id=".$rvvalue['albumId'];
                $sth1 = $dbh->prepare($sql1);
                $sth1->execute();
                $albumobj = $sth1->fetchObject();
                
                $rvvalue['album_image'] = $albumobj->album_image_path."/".$albumobj->album_image;
            }else{
                $rvvalue['album_image'] ="";
            }
            $rv[$i]=$rvvalue;
    ...
    public function getLog(int $skip, int $take){
        $dbh = $this->getDb();
    ...
    public function insertApplication($app, $userId, $conf_uid){
        ...
        $sql = "INSERT INTO application (UserId, FirstName,  LastName, DOB, City, StateOrProvince, Country, Email, SongsReview, AppConfirmUID, AppDate, MusicServed, Approval_Status,isupload)";
        $sql .= " VALUES (:userId, :firstName, :lastName, :dob, :city, :stateOrProvince, :country, :email, :songsReview, :appConfirmUid, NOW(), :musicServed, 'P',:uploadasong)"; 
        ...
        if($app->uploadasong == "No"){
            ...
            $sql = "INSERT INTO user_pkgs(UserId,PkgId, CCStatus,ExpDate) VALUES(:userId, :pkgId, :CCStatus,:ExpDate)";
            $sth = $dbh->prepare($sql);
...
*/
    ...
    public function insertSong($song, int $userId){
        if($song->copyrightYear == ""){
            ...
            $song->copyrightYear =substr($song->copyrightYear, 0, strlen($song->copyrightYear) - (strlen($song->copyrightYear) - 4) );
        }
        if($song->albumId == ""){
            $song->albumId=null;
            $Status = 'QA';
             ...
             $sql = 'UPDATE songs
                    SET SongTitle = :songTitle, ArtistName = :artistName, Genre = :genre, Label = :label, 
                        ...
                        `Facebook_link` = :Facebook_link, `Twitter_link` = :Twitter_link, `Spotify_link` = :Spotify_link,
                         `iTunes_link` = :iTunes_link, artistcity = :artistcity, sociallink = :sociallink
                    WHERE SongId = :songId';
        $dbh = $this->getDb();
        $sth = $dbh->prepare($sql);
        $sth->execute(
    ...
    public function getdeletesongs(int $userId,int $songId){
        ...
        if($rv1->Status == "QA"){
            ...
            $artist_imagepath="../images/artist_images/".$rv1->artist_image;
            unlink($artist_imagepath);
            $SongFile = "../$rv1->SongPath/$rv1->SongFile";
            unlink($SongFile);
            $SampleFile = "../$rv1->SongPath/$rv1->SampleFile";
            unlink($SampleFile);
   ...
   public function SongApprove($filterby,int $skip, int $take){
       ...
       if( $filterby == "Pending" ){
            $sql = "SELECT  s.SongId, s.UserId, s.SongFile, s.SongPath, s.SongTitle, s.ArtistName, s.Duration, Go.Name   Genre,s.SubGenre, s.Label, s.State, s.Country, s.Website, s.CD, s.Vinyl, s.MIDownload, s.Status, 
                       s.tire2status, s.Region, s.Dist_Type, s.DJAssociation, s.Normal_Dist, s.DistDate, s.artist_image,
                        s.AlbumName, s.Copyright_Owner, s.Copy_Year, s.wheretobuy, s.sub_distype, s.fans, s.SampleFile, 
                        s.Filename , s.Facebook_link, s.Twitter_link, s.Spotify_link, s.iTunes_link,
                        s.albumId , albm.album_type , albm.album_name ,albm.genre as album_gener, IFNULL(s.albumId,RAND()) as new_albumId 
                    FROM songs s
                    JOIN genresofficial Go ON Go.Id = s.Genre
                    LEFT JOIN  album albm on albm.album_id =  s.albumId 
                    WHERE s.Status='QA' group by new_albumId  order by s.SongId DESC
            ...
            $sql = "SELECT  s.SongId, s.UserId, s.SongFile, s.SongPath, s.SongTitle, s.ArtistName, s.Duration, Go.Name Genre,s.SubGenre, s.Label, s.State, s.Country, s.Website, s.CD, s.Vinyl, s.MIDownload, s.Status, 
                    ...
                    JOIN genresofficial Go ON Go.Id = s.Genre
                    LEFT JOIN  album albm on albm.album_id =  s.albumId 
                    where s.Status='A' and s.SongId IN (SELECT SongId FROM `song_feedback` where OverallRating > 0)
                    group by new_albumId  order by s.SongId DESC
                    LIMIT :skip, :take";
            ...
            $sql = "SELECT  s.SongId, s.UserId, s.SongFile, s.SongPath, s.SongTitle, s.ArtistName, s.Duration, Go.Name Genre,s.SubGenre, s.Label, s.State, s.Country, s.Website, s.CD, s.Vinyl, s.MIDownload, s.Status, 
                    ...
                    JOIN genresofficial Go ON Go.Id = s.Genre
                    LEFT JOIN  album albm on albm.album_id =  s.albumId
                    where s.Status='A' and s.SongId NOT IN (SELECT SongId FROM `song_feedback`)
                    group by new_albumId  order by s.SongId DESC
                    LIMIT :skip, :take";
         ...
         //, avg(sf.OverallRating) as OverallRating , count(sf.Id) as NoOfRaters            
        $sth = $dbh->prepare($sql);
        $sth->bindValue(':skip', $skip, PDO::PARAM_INT);
        $sth->bindValue(':take', $take, PDO::PARAM_INT);
        $sth->execute();
        ...
        foreach ($rv as $rvvalue) {
                ...
                $sql1 = "SELECT count(Id) as NoOfRaters,IFNULL(AVG(OverallRating),0) as OverallRating FROM song_feedback where OverallRating > 0 and SongId =".$rvvalue['SongId'];
                $sth1 = $dbh->prepare($sql1);
                $sth1->execute();
                $ratingobj = $sth1->fetchObject();
                
                $rvvalue['OverallRating'] = $ratingobj->OverallRating;
                $rvvalue['NoOfRaters'] = $ratingobj->NoOfRaters;
    ...
    public function getsongcrateaddedcountinuser($userId,$songId){
        ...
        $sth->execute();
        $rv = $sth->fetchObject();
        $djcratecount = $rv->total;

        $sql1 = "SELECT COUNT(*) totalfeedback FROM song_feedback sf 
                WHERE sf.OverallRating > 0 and sf.SongId = ".$songId." and sf.DJUserId=".$userId."";


        $sth1 = $dbh->prepare($sql1);
        $sth1->execute();
        $rv1 = $sth1->fetchObject();
        $totalfeedback = $rv1->totalfeedback;

        if( $djcratecount > 0 && $totalfeedback > 0){
            return "1";
        }else if($djcratecount > 0 && $totalfeedback == 0){
            return "2";
        }else if($djcratecount == 0 && $totalfeedback > 0){
            return "3";
        }else{
    ...
    public function getSongcraterequests($userid,int $skip, int $take)
    {
        ...
        $sql = "SELECT sc.crate_id,u.UserId,u.Username,sc.created_date,sc.status 
                FROM songcrate sc 
                join users u on sc.to_userid = u.UserId 
                WHERE sc.status != 'Approved' and sc.from_userid = ".$userid." LIMIT $skip, $take";
    ...
    public function getSongcraterequestCounts($userid){
        ...
        $dbh = $this->getDb();
         $sql = "SELECT COUNT(*) total FROM `songcrate` WHERE status != 'Approved' and from_userid = ".$userid;
        $sth = $dbh->prepare($sql);
        $sth->execute();
        $rv = $sth->fetchObject();
        return $rv->total;
    ...
    public function getalbumsonguploadsCountByUser($albumId,$albumtype,$userId){
        ...
        $sql = "SELECT  s.SongId, s.UserId, s.SongFile, s.SongPath, s.SongTitle, s.ArtistName, s.Duration, Go.Name Genre,
                       ...
                       s.tire2status, s.Region, s.Dist_Type, s.DJAssociation, s.Normal_Dist, s.DistDate, s.artist_image,
                        s.AlbumName, s.Copyright_Owner, s.Copy_Year, s.wheretobuy, s.sub_distype, s.fans, s.SampleFile, 
                        s.Filename , s.Facebook_link, s.Twitter_link, s.Spotify_link, s.iTunes_link,
                        s.albumId , albm.album_type , albm.album_name ,albm.genre as album_gener
                    FROM songs s
                    JOIN genresofficial Go ON Go.Id = s.Genre
                    LEFT JOIN  album albm on albm.album_id =  s.albumId 
                    WHERE s.albumId=".$albumid." order by s.SongId DESC
                    LIMIT :skip, :take";
    ...
    public function getSiteconfiginfo(){
        $dbh = $this->getDb();
        $sql ="SELECT id, ep_limit, album_limit, ideltime, defaultpackage, defaultreviewlimit, fileuploadsize, listeningroomconfig, paypal_env, paypal_key, tire2_per, tire2_ret, tire3_per, tire3_ret, tire4_per, tire4_ret FROM siteconfig WHERE id=1";
        $sth = $dbh->prepare($sql);
        $sth->execute();
        $rv = $sth->fetchObject();
        return $rv;
    ...
    public function updateSiteconfiginfo($Siteconfig){
        ...
        $sql = 'UPDATE siteconfig
                    ...
                    paypal_env = :paypal_env, paypal_key = :paypal_key, tire2_per = :tire2_per, 
                    tire2_ret = :tire2_ret, tire3_per = :tire3_per, tire3_ret = :tire3_ret, 
                    tire4_per = :tire4_per, tire4_ret = :tire4_ret
                    WHERE id = 1';
        $dbh = $this->getDb();
        $sth = $dbh->prepare($sql);
        $sth->execute(
            [
                ...
                ':tire2_per'=> $Siteconfig->tire2_per,
                ':tire2_ret'=> $Siteconfig->tire2_ret,
                ':tire3_per'=> $Siteconfig->tire3_per,
                ':tire3_ret'=> $Siteconfig->tire3_ret,
                ':tire4_per'=> $Siteconfig->tire4_per,
                ':tire4_ret'=> $Siteconfig->tire4_ret,
                ]);
        $rv = $sth->rowCount();
        /*$sth->debugDumpParams();
        exit;*/
    ...
    public function getadminrights($userId){
        $dbh = $this->getDb();
        $sql = "SELECT userId, application, music, sitesetting, users, auditlog, logs,mediadistribution,packagesetting
 ...
 public function insertadminrights($editadminrights){
        ...
        if($editadminrights->packagesetting == ""){
            $editadminrights->packagesetting=0;
        }


        $sql = "INSERT INTO users_rights(userId, application, music, sitesetting, users, auditlog, logs,mediadistribution,packagesetting) 
        VALUES ('$editadminrights->userId','$editadminrights->applications','$editadminrights->music','$editadminrights->siteconfig','$editadminrights->users','$editadminrights->auditlogs','$editadminrights->logs','$editadminrights->mediadistribution','$editadminrights->packagesetting')";
        $dbh = $this->getDb();
        $sth = $dbh->prepare($sql);
        $sth->execute();
        $rv = $sth->rowCount();
    ...
    public function updateadminrights($editadminrights){
        ...
        $sth->execute(
            [
                ...
                ':packagesetting'=> $editadminrights->packagesetting,
                ]);
        $rv = $sth->rowCount();
        /*$sth->debugDumpParams();
        exit;*/
    ...
    public function updateUserpackage($packageId,$userId){
       ...
       //$packagedetail= $this->getPackage($packageId);
        ...
        if($packageId == 117){
            $sql1 = "UPDATE application 
                    SET isupload = 'No'
                    WHERE UserId = $userId";
            $sth1 = $dbh->prepare($sql1);
            $sth1->execute();
        }else{
            $sql12 = "UPDATE application 
                    SET isupload = 'Yes'
                    WHERE UserId = $userId";
...
public function getSongsToReviewgener($userId){
        ...
        $sql = "SELECT  DISTINCT(s.Genre) ,Go.Name GenreName FROM songs s
                    ...
                    LEFT JOIN song_feedback sf ON s.SongId = sf.SongId AND sf.DJUserId = :userId
                    LEFT JOIN  album albm on albm.album_id =  s.albumId
                    WHERE sf.OverallRating = 0 and date(s.DistDate) BETWEEN '".$beforedate."' AND '".$currentdate."' ORDER BY GenreName ASC ";  
        ...
        foreach ($rv as $rvvalue) {
           ...
           $sql = "SELECT  s.SongId, s.UserId, u.Username, s.SongFile, s.SongPath, s.SongTitle, s.ArtistName, s.Duration, Go.Name Genre,
                   ...
                   group by new_albumId ORDER BY s.DistDate DESC ";    
                    ...
                    $sth->bindValue(':userId', $userId, PDO::PARAM_INT);
                    $sth->execute();
                    $rv1=$sth->fetchAll(PDO::FETCH_ASSOC);
                    ...
                    foreach ($rv1 as $rvvalue1) {
                        if($rvvalue1['albumId'] != ""){
                            $sql1 = "SELECT album_image,album_image_path FROM album 
                                  WHERE album_id=".$rvvalue1['albumId'];
                            $sth1 = $dbh->prepare($sql1);
                            $sth1->execute();
                            $albumobj = $sth1->fetchObject();
                            
                            $rvvalue1['album_image'] = $albumobj->album_image_path."/".$albumobj->album_image;
                        }else{
                            $rvvalue1['album_image'] ="";
                        }
                        $rv1[$j]=$rvvalue1;
 ...
 public function deletesongsbyalbum(int $userId,int $albumId){
    ...
    /*{ id: 8, name: 'White', parentId: 5 }*/
        ...
        $dbh = $this->getDb();
        $sql = "SELECT * FROM `genresofficial` WHERE `parent_id` = 0 ORDER BY `Name` ASC";
        $sth = $dbh->prepare($sql);
        $sth->execute();
        //$rv = $sth->fetchObject();
        $rv = $sth->fetchAll(PDO::FETCH_ASSOC);
        ...
        foreach ($rv as $rvvalue) {
            ...
            $sql1 = "SELECT * FROM `genresofficial` WHERE `parent_id` = ".$rvvalue['Id']." ORDER BY `Name` ASC";
            $sth1 = $dbh->prepare($sql1);
            $sth1->execute();
            $rv1 = $sth1->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function getSubGenreByid($generid){
        $dbh = $this->getDb();
        $sql = "SELECT * FROM `genresofficial` WHERE `parent_id` = $generid ORDER BY `Name` ASC";
        $sth = $dbh->prepare($sql);
        $sth->execute();
        //$rv = $sth->fetchObject();
        $rv = $sth->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function packageregisterApplication($packageId,$appId,$userId){
         $dbh = $this->getDb();
        $end = date('Y-m-d', strtotime('+1 years'));
       //$packagedetail= $this->getPackage($packageId);
        $sql = "SELECT ap.* FROM account_packages ap  WHERE ap.PkgId = '".$packageId."'";
    ...
    public function getAppPackagesList(){
        $dbh = $this->getDb();
        $min = SONGBOOK_PKGID;
        if (IS_SONGBOOK == 0) $min++;
        ...
        $sql ="SELECT 
                    ...
                    Concat(PkgDetail,' ', 
                        (
                            IF(Price >= 0, 
                                Concat('(\$', Price, '; ', IF(Songs = -1, 'unlimited', Songs), ' MB',')'), ''))) PkgDisplayName
                    FROM account_packages
                    WHERE PkgType = 'Music Pro' and PkgId >= $min";
        $sth = $dbh->prepare($sql);
        $sth->execute();
        $rv = $sth->fetchAll(PDO::FETCH_ASSOC);
        $dbh = null;
    ...
    public function getAppforvarificationemail($userId){
        $dbh = $this->getDb();
    ...
    public function checkconfirmEmail(int $conf_uid, int $userId){
        $dbh = $this->getDb();
        $sth = $dbh->prepare('SELECT * from `application` WHERE AppConfirmUID = :confUid AND UserId = :userId and Approval_Status = :status');
        $sth->execute([':userId'=> $userId, ':confUid'=> $conf_uid, ':status'=> 'A']);
        $rv = $sth->fetchObject();
        return $rv;

    }
    public function checkconfirmapplicationUserId(int $userId)
    {
        $dbh = $this->getDb();
        $sth = $dbh->prepare('SELECT AppId from `application` WHERE  UserId = :userId ');
        $sth->execute([':userId'=> $userId ]);
        $rv = $sth->fetchObject();
        return $rv->AppId;
    }
    public function getGenreNamewheneditsong($userId,$songid)
    {
        $dbh = $this->getDb();
        $sth = $dbh->prepare("SELECT gn.Id as genre FROM songs s LEFT JOIN album al ON al.album_id = s.albumId LEFT JOIN genresofficial gn ON gn.Name = al.genre WHERE s.SongId = :songid and s.UserId = :userId ");
        $sth->execute([':userId'=> $userId,':songid'=> $songid ]);
        $rv = $sth->fetchObject();
        return $rv->genre."|";
    }
     public function updatedeletedateApplication($appId){
        //print_r($appId);
        $closedate=date("Y-m-d");
        $dbh = $this->getDb();
        $sql = "UPDATE application SET closedate = '".$closedate."'
                    WHERE AppId = ".$appId."";
     ...
     public function admingetpackagedetails(){
        $dbh = $this->getDb();
        $sql ="SELECT * FROM account_packages";
        $sth = $dbh->prepare($sql);
        $sth->execute();
        $rv = $sth->fetchAll(PDO::FETCH_ASSOC);
        return $rv;
    }
    public function editpackageinfo($packagedetail){
        

        $sql="UPDATE account_packages 
                SET Price = :Price, 
                Songs = :Songs 
                where PkgId = :PkgId";
        $dbh = $this->getDb();
        $sth = $dbh->prepare($sql);
        $sth->execute([':Price'=> $packagedetail->Price, 
                        ':Songs'=> $packagedetail->mb, 
                        ':PkgId'=> $packagedetail->PkgId]);
        return 1;
         
    }
    public function getlargestpackageid(){
        $dbh = $this->getDb();
        $sql ="SELECT * FROM account_packages WHERE PkgType = 'Music Pro' ORDER BY PkgId DESC LIMIT 0,1";
        $sth = $dbh->prepare($sql);
        $sth->execute();
        $rv = $sth->fetchObject();
    ...
    public function getAuditlogCount(){
        $dbh = $this->getDb();
        $sql = 'SELECT COUNT(*) total FROM auditlog';
        $sth = $dbh->prepare($sql);
        $sth->execute();
        $rv = $sth->fetchObject();
        return $rv->total;
    ...
    public function getlogCount(){
        $dbh = $this->getDb();
        $sql = 'SELECT COUNT(*) total FROM log';
        $sth = $dbh->prepare($sql);
        $sth->execute();
        $rv = $sth->fetchObject();
        return $rv->total;
    }
        public function getUswersGenreGroupNamesbyuser($userId){
        $dbh = $this->getDb();
        $usersql="SELECT `MusicServed` FROM `application` WHERE `UserId` = $userId";
        $sthuser = $dbh->prepare($usersql);
        $sthuser->execute();
      ...
      $sql = "SELECT * FROM `genresofficial` WHERE  `Name` IN ('".$usermusicserved."')  group by `parent_id` ORDER BY `parent_id` ASC";
        ...
        foreach ($rv as $rvvalue) {
            ...
            if($sth1->rowCount() > 0){
                ...
                $genresofficial[$i]['name']=$parentuser->Name;
                $genresofficial[$i]['isLabel']=true;
                $subgenresofficial=array();
                $j=0;
                    foreach ($rv1 as $rvvalue1) {
                        if(in_array($rvvalue1['Name'], $usermusicserved1))
                        {
                            $subgenresofficial[$j]['id']=$rvvalue1['Id'];
                            $subgenresofficial[$j]['name']=$rvvalue1['Name'];
                            $subgenresofficial[$j]['isLabel']=false;
                            $subgenresofficial[$j]['parentId']=$rvvalue['Id'];
                            $genresofficial[$i]['subcategory']=$subgenresofficial;
                            $j++;
