...
function Header($myheader='')
{
	...
	$this->Cell(0,0.5,$myheader,0,2,'L');
	$this->Ln(0.375);
}
 
function Footer($myfooter='')
{
	//To be implemented in your own inherited class

	$this->SetFont('Arial', 'U', '8');
	$this->Ln(0.375);
	$this->Cell(0,0.5,$myfooter,0,2,'C');
}
...
//width="378"  height="64"
function Image($file,$x,$y,$w=1,$h=1,$type='',$link='')
{
...
if(isset($HTTP_SERVER_VARS['HTTP_USER_AGENT']) and $HTTP_SERVER_VARS['HTTP_USER_AGENT']=='contype')
{
	...
	function getisofriday($year, $week) 
	{
       # check input
       $year = min ($year, 2038); $year = max ($year, 1970);
       $week = min ($week, 53); $week = max ($week, 5);
       # make a guess
       $friday = mktime (1,1,1,1,7*$week,$year);
       # count down to week
       while (strftime('%V', $friday) != $week)
               $friday -= 60*60*24*7;
			   //$monday += 60*60*24*7;
       # count down to monday
       while (strftime('%u', $friday) != 5)
               $friday -= 60*60*24;
			   //$monday += 60*60*24;
       # got it
       return $friday;
