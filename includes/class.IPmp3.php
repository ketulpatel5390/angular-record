...
class mp3 {
	...
	private function get_id3v2() {
		...
		//It appears the header is not included in the size so add 10 bytes to $pos_end
		$pos_end = $pos_start + $tagsize;
	...
	private function get_data() {
		...
		$padding_data = fread($this->fp, 4096);

		//$padding_size = @max(0, strpos($padding_data, trim($padding_data)));
        $padding_size = $this->findSyncWord($padding_data);
    ...
    private function findSyncWord(string $padding_data){
        $rv = 0;
        $syncWordFound = false;
        $length = strlen($padding_data);
        while (!$syncWordFound && $rv < $length){
            $frameheader = substr($padding_data, $rv, 4);
            $frameheaderObject = $this->get_frameheader($frameheader);
            if ($frameheaderObject){
                //confirm by checking next frame
                $frameheader = substr($padding_data, $rv + $frameheaderObject->framesize, 4);
                $frameheaderObject = $this->get_frameheader($frameheader);
                $syncWordFound = $frameheaderObject !== false;
            }
            if (!$syncWordFound) $rv++;
        }
	...
	private function get_data_frames() {
		...
		while(1) {
			...
			if(!$frameheader = $this->get_frameheader($frameheaders)) {
                //attempt to resync
                fseek($this->fp, $pos_frame - 3);
                $buffer = fread($this->fp, 4096);
                $pos = $this->findSyncWord($buffer);
                if ($pos >= strlen($buffer))
                    break;
                else {
                    fseek($this->fp, $pos_frame - 3 + $pos);
                    continue;
                }
			}
